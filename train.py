...
if 1:
    ...
    train         = EasyDict(run_func_name='training.training_loop.training_loop')         # Options for training loop. 用于训练的主函数
    G             = EasyDict(func_name='training.networks_stylegan.G_style')               # Options for generator network. 生成网络
    D             = EasyDict(func_name='training.networks_stylegan.D_basic')               # Options for discriminator network. 判别网络
    G_opt         = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                          # Options for generator optimizer. 生成网络优化函数参数
    D_opt         = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                          # Options for discriminator optimizer.
    G_loss        = EasyDict(func_name='training.loss.G_logistic_nonsaturating')           # Options for generator loss. 生成网络损失函数参数
    D_loss        = EasyDict(func_name='training.loss.D_logistic_simplegp', r1_gamma=10.0) # Options for discriminator loss.
    dataset       = EasyDict()                                                             # Options for load_dataset(). 数据集
    sched         = EasyDict()                                                             # Options for TrainingSchedule. 训练的计划表
    grid          = EasyDict(size='4k', layout='random')                                   # Options for setup_snapshot_image_grid(). 快照图片网格参数
    metrics       = [metric_base.fid50k]                                                   # Options for MetricGroup. 评估函数
    # 在 dnnlib 的 __init__.py 脚本中导入
    submit_config = dnnlib.SubmitConfig()                                                  # Options for dnnlib.submit_run().
    tf_config     = {'rnd.np_random_seed': 1000}                                           # Options for tflib.init_tf().

    # Dataset.
    # desc += '-jellyfish';  dataset = EasyDict(tfrecord_dir='jellyfish'); train.mirror_augment = True
    desc += '-horse';  dataset = EasyDict(tfrecord_dir='horse'); train.mirror_augment = True
...
def main():
    # kwargs 是一个重写后的Dict类型，继承了所有Dict的变量和函数
    ...
    # 提交，运行
    # 不明白为啥没有传前面两个参数
    # kwargs是个字典，加上**之后就以关键字参数进行传递过去 
    # Python允许你在list或tuple前面加一个*号，把list或tuple的元素变成可变参数传进去；加两个**，把dict变成关键字参数传进去
