...
def create_celeba(tfrecord_dir, celeba_dir, cx=89, cy=121):
    ...
    glob_pattern = os.path.join(celeba_dir, 'images', '*.jpg')
    print(glob_pattern)
    # glob_pattern = os.path.join(celeba_dir, 'img_align_celeba_png', '*.png')
    image_filenames = sorted(glob.glob(glob_pattern))
    expected_images = 202599
    if len(image_filenames) != expected_images:
        print('[INFO] Dataset length:{}'.format(len(image_filenames)))
        error('Expected to find %d images' % expected_images)

    # new一个TFRecordExporter对象来转换数据
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        order = tfr.choose_shuffled_order()
        for idx in range(order.size):
            img = np.asarray(PIL.Image.open(image_filenames[order[idx]]))
            # CelebA每张图片尺寸都是218*178，如果不是，则程序退出
...
if __name__ == "__main__":
    # python dataset_tool.py create_lsun datasets/lsun-bedroom-full ~/Data/LSUN/bedroom_lmdb --resolution 256
    # python dataset_tool.py create_celeba datasets/celeba /Users/xufeng/Data/CelebA 
    # python dataset_tool.py create_celeba datasets/celeba /home/xufengchen/Data/CelebA 
    # python dataset_tool.py create_from_images datasets/jellyfish /Users/xufeng/Data/Jellyfish/recognizer/true_filter_256
    # python dataset_tool.py create_from_images datasets/jellyfish /home/xufengchen/Data/Jellyfish/recognizer/true_filter_256
